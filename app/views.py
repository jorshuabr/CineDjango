...
def generos(request):
	...
	#Se obtienen y muestran las preguntas ordenadas por numero de votos, de mayor a menor
		...
		for film in film_lista:
			if film.genero not in generos:
				generos.append(film.genero)
		
		film_lista = Pelicula.objects.none()
		if 'genero' in request.GET:
			film_lista = Pelicula.objects.all().order_by('-votos').filter(genero=request.GET.get('genero'))
		paginator = Paginator(film_lista, 4) #Se indica que se mostraran 4 peliculas por pagina
		page = request.GET.get('page')
		try:
			pelis = paginator.page(page)
			...
			#Si esta fuera del rango se muestra la ultima
			pelis = paginator.page(paginator.num_pages)
		return render(request, 'app/genero.html', {'pelis':pelis, 'generos' : generos})
